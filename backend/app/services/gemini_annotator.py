...
from ..model_config import api_key_for, resolve_model
...
def annotate_file(path: Path, style: str, model_id: str | None = None):
    model = resolve_model(model_id)
    api_key = api_key_for(model)
    if not api_key: raise RuntimeError(f"{model['api_key_env']} is not configured")
    try:
    ...
    response = client.models.generate_content(model=model["model_name"], contents=[prompt, types.Part.from_bytes(data=path.read_bytes(), mime_type="image/jpeg")], config=types.GenerateContentConfig(response_mime_type="application/json", response_json_schema=SCHEMA))
